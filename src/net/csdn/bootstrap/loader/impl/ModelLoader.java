...
public class ModelLoader implements Loader {
    ...
    public void load(Settings settings) throws Exception {
        final Enhancer enhancer = new JPAEnhancer(ServiceFramework.injector.getInstance(Settings.class));
        final List<CtClass> classList = new ArrayList<CtClass>();
        ServiceFramework.scanService.scanArchives(settings.get("application.model"), new ScanService.LoadClassEnhanceCallBack() {
            @Override
