...
//            config.setMaxActive(1000);
            pool = new JedisPool(config,
                    settings.get(ServiceFramework.mode + ".datasources.redis.host"),
                    settings.getAsInt(ServiceFramework.mode + ".datasources.redis.port", 6379));
        } catch (Exception e) {
            Log.error("cache init failed, %s", e.getMessage());
        }
    }
